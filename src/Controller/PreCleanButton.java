...
public class PreCleanButton {
    ...
    public static void onClick() {
        ...
        if (convertedFile != null) {
            try {
                parsedSamples = parseCSV(convertedFile, propWorker.getProperties());
            } catch (Exception ex) {
    ...
    /**
     * Brings up an open file dialog box for the user to select a CSV file to
     * open.
     *
     ...
     * @param conversionMinimums the conversion minimums from the JSON config
     * file
     * @param conversionEquations the conversion equations from the JSON config
     * file
     ...
     * @throws IOException
     */
    private static SampleAndCheck parseCSV(File csvFile, Properties properties) throws FileNotFoundException, IOException {
        SunshineConversion conversion = new SunshineConversion(properties);
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (preCleanIndex.isComplete()) {
               ...
               ==================== */
                Sample tmp = new Sample(preCleanIndex, pieces, (Double.parseDouble(properties.getProperty("dilutionFactor"))), conversion);
                if (tmp.isCheck()) {
                    /*====================
